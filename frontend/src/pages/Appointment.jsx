...
const Appointment = () => {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await axios.get('http://localhost:5000/doctor/find-all');
      setDoctors(response.data); // Sử dụng response để cập nhật danh sách bác sĩ
    } catch (error) {
      console.error('Error fetching doctors:', error);
    }
  };
  ...
  const handleBookAppointment = async () => {
    ...
    if (!selectedSlot) return alert("Vui lòng chọn thời gian hợp lệ!");

    const appointmentData = {
      doctorId: docId,
      appointmentTime: selectedSlot.datetime,
      // Thêm thông tin cần thiết khác nếu cần
    };

    try {
      await axios.post('http://localhost:5000/appointment_history/create', appointmentData);
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length > 0 && docSlots.map((item, index) => (
              <div 
                ...
                key={index}
              >
                <p>{item[0] && item[0].datetime.getDate()}/{item[0] && item[0].datetime.getMonth() + 1}</p>
              </div>
            ))
          }
        </div>
